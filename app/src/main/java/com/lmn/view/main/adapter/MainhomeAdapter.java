     ...
     helper.getView(R.id.al_message).setOnClickListener(new View.OnClickListener() {
         ...
         public void onClick(View v) {
             ARouter.getInstance().build("/detail/activity")
                     ...
                     .withString("status", item.getId()+"")
                     .navigation();
         }
     });
        ImageFactory.getLoader().loadNet(mContext.getApplicationContext(),(ImageView) helper.getView(R.id.img_left),item.getImg(),new ILoader.Options(R.drawable.loading_img,R.drawable.error_img));
    }
